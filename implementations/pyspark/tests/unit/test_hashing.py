...
@patch("kimball.hashing.sha2")
@patch("kimball.hashing.concat_ws")
@patch("kimball.hashing.when")
...
@patch("kimball.hashing.col")
@patch("kimball.hashing.lit")
def test_compute_hashdiff_single_column(
    mock_lit, mock_col, mock_when, mock_concat_ws, mock_sha2
):
    """Test hashdiff with single column."""
    from kimball.hashing import compute_hashdiff
    ...
    _ = compute_hashdiff(["id"])

    # col() is called twice per column: isNull check + cast fallback
...
@patch("kimball.hashing.col")
@patch("kimball.hashing.lit")
def test_compute_hashdiff_empty_columns(
    mock_lit, mock_col, mock_when, mock_concat_ws, mock_sha2
):
    """Test hashdiff with no columns."""
    from kimball.hashing import compute_hashdiff
    ...
    # Should still call concat_ws and sha2, just with no columns
