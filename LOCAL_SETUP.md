...
|---------|------------|-------------|
| **Surrogate keys** | `GENERATED BY DEFAULT AS IDENTITY` (auto-increment) | `xxhash64(natural_keys + hashdiff)` (deterministic hash, unique per version) |
...
2. The `IdentityKeyGenerator` isn't generating version-unique keys (fixed: combines hashdiff with natural key hash)