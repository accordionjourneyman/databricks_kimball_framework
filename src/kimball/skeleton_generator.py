...
class SkeletonGenerator:
    ...
    def generate_skeletons(self, 
        ...
        if not spark.catalog.tableExists(dim_table_name):
            print(f"Dimension table {dim_table_name} does not exist. Skipping skeleton generation.")
            return
        ...
        skeletons = skeletons.withColumn("__is_current", lit(True)) \
        ...
        for field in target_schema.fields:
            if field.name not in skeletons.columns and field.name != surrogate_key_col:
                skeletons = skeletons.withColumn(field.name, lit(None).cast(field.dataType))
        ...
        if surrogate_key_strategy == "identity":
            ...
            skeletons = skeletons.withColumn(surrogate_key_col, xxhash64(col(dim_join_key).cast("string")))
