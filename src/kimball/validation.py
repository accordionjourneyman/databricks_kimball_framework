...
class TestResult:
    ...
    def __str__(self) -> str:
        icon = "✅" if self.passed else ("⚠️" if self.severity == TestSeverity.WARN else "❌")
...
class ValidationReport:
    ...
    def error_count(self) -> int:
        ...
        return sum(1 for r in self.results if not r.passed and r.severity == TestSeverity.ERROR)

    @property
    def warning_count(self) -> int:
        """Count of failed WARN-severity tests."""
        return sum(1 for r in self.results if not r.passed and r.severity == TestSeverity.WARN)
    ...
    def __str__(self) -> str:
        ...
        lines.append("=" * 40)
        lines.append(f"Summary: {self.error_count} errors, {self.warning_count} warnings")
        return "\n".join(lines)

    def raise_on_failure(self) -> None:
        """Raise DataQualityError if any ERROR-severity tests failed."""
        if not self.passed:
            failed = [r for r in self.results if not r.passed and r.severity == TestSeverity.ERROR]
...
class DataQualityValidator:
    ...
    def _sample_failures(self, bad_df: DataFrame, sample_size: int, sample_fn: Callable[[DataFrame, int], list[dict[str, Any]]] | None = None) -> list[dict[str, Any]]:
        if sample_size <= 0:
            return []
    ...
    def _build_test_result(
        ...
        samples = self._sample_failures(bad_df, sample_size, sample_fn) if bad_count != 0 else []
        return TestResult(
            test_name=test_name,
            passed=bad_count == 0,
    ...
    def _test_error(self, test_name: str, severity: TestSeverity, e: Exception) -> TestResult:
        return TestResult(
            test_name=test_name,
            passed=False,
            severity=severity,
    ...
    def validate_relationships(
        ...
        test_name = f"relationships({fk_column} -> {reference_table}.{reference_column})"
        try:
    ...
    def validate_natural_key_uniqueness(
        ...
        try:
            ...
            if is_dev_mode:
                ...
                duplicates_check = df.groupBy(*natural_keys).count().filter(F.col("count") > 1)
                duplicate_count = 0 if duplicates_check.limit(1).isEmpty() else -1
                total_rows = -1
            details = None
            sample_failures: list[dict[str, Any]] = []
            if duplicate_count != 0:
                duplicates = df.groupBy(*natural_keys).agg(F.count("*").alias("_dup_count")).filter(F.col("_dup_count") > 1).orderBy(F.col("_dup_count").desc()).limit(5)
            ...
            return TestResult(test_name=test_name, passed=duplicate_count == 0, failed_rows=duplicate_count, total_rows=total_rows, severity=severity, details=details, sample_failures=sample_failures)
        except Exception as e:
            return self._test_error(test_name, severity, e)

    def _check_single_fk(self, df: DataFrame, fk: dict[str, Any], exclude_seeds: bool, severity: TestSeverity) -> TestResult | None:
