...
def merge_scd1(
    ...
    *,
        ...
        try:
            ...
            if data_cols:
                ...
                target_hashed = target_df.select(
                    *join_keys, *[c for c in data_cols if c in target_df.columns]
                ).withColumn("_hash", compute_hashdiff(data_cols))
    ...
    merge_builder.whenMatchedUpdate(
        condition=cast(Any, update_cond), set=cast(Any, update_map)
    ).whenNotMatchedInsert(
        condition=cast(Any, update_cond), values=cast(Any, insert_map)
    ).execute()
