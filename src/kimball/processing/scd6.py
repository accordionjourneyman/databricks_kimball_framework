...
def merge_scd6(
    ...
    *,
    ...
    insert_rows_with_sk = HashKeyGenerator(
        join_keys, version_column=effective_at_column
    ).generate_keys(insert_rows, surrogate_key_col)
    non_insert_rows = staged_final.filter(col("__action") != "INSERT")
    staged_final = insert_rows_with_sk.unionByName(
        non_insert_rows, allowMissingColumns=True
    )
