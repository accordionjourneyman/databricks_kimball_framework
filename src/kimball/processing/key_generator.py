...
class HashKeyGenerator(KeyGenerator):
    def __init__(self, natural_keys: list[str], allow_for_dimension_surrogates: bool = False):
        import warnings
        if not allow_for_dimension_surrogates:
            warnings.warn("HashKeyGenerator should not be used for dimension surrogate keys.", UserWarning, stacklevel=2)
...
class SequenceKeyGenerator(KeyGenerator):
    def generate_keys(self, df: DataFrame, key_col_name: str, existing_max_key: int = 0) -> DataFrame:
        ...
        return df.withColumn(key_col_name, row_number().over(Window.orderBy(lit(1))) + lit(existing_max_key))
