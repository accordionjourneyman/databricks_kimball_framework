    ...
    def create_table_with_clustering(self, 
        ...
        for field in schema_df.schema.fields:
            ...
            if col_name == surrogate_key_col and surrogate_key_strategy == "identity":
                # Use GENERATED BY DEFAULT AS IDENTITY - allows explicit inserts (for default rows -1,-2,-3)
                # but auto-generates when value is not provided
                col_def = f"{col_name} BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1)"
            else:
