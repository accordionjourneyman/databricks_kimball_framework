...
class DeltaMerger:
    ...
    def _merge_scd1(self, target_table_name, source_df, join_keys, delete_strategy, schema_evolution, surrogate_key_col=None, surrogate_key_strategy="identity"):
        ...
        if surrogate_key_col and surrogate_key_strategy:
            ...
            if surrogate_key_strategy == "identity":
                ...
                try:
                    max_key_row = delta_table.toDF().agg({surrogate_key_col: "max"}).collect()[0][0]
    ...
    def _merge_scd2(self, target_table_name, source_df, join_keys, track_history_columns, surrogate_key_col, surrogate_key_strategy, schema_evolution: bool = False):
        """
        Implements SCD Type 2 Merge using Hashdiff and Surrogate Keys.
        """
    ...
    def ensure_scd2_defaults(self, target_table_name, schema, surrogate_key, default_values=None):
        ...
        existing_defaults = delta_table.toDF().filter(col(surrogate_key).isin([-1, -2, -3])).select(surrogate_key).collect()
        existing_keys = {row[surrogate_key] for row in existing_defaults}
