...
def main() -> int:
    parser = argparse.ArgumentParser(description="Monitor Kimball upstream data contracts")
    parser.add_argument("--config-glob", required=True)
    parser.add_argument("--etl-schema", required=True)
    args = parser.parse_args()
    result = ContractMonitor(glob.glob(args.config_glob, recursive=True), get_spark(), args.etl_schema).run()
    print(result)
