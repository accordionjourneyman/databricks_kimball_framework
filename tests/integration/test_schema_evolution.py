...
"""
        ...
        # Verify table created with IDENTITY
        table_schema = spark.table("test_db.dim_customer").schema
        sk_field = next(f for f in table_schema.fields if f.name == "customer_sk")
        assert "GENERATED ALWAYS AS IDENTITY" in str(sk_field.dataType)
